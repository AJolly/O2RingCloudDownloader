...
def compute_summary(events: List[SpikeEvent], valid: np.ndarray,
    ...
    return NightSummary(
        ...
        spike_index=round(si, 2),
        prri_count=prri_count,
        prri_index=int(round(prri_index)),
        mean_delta_hr=round(mean_d, 1),
        median_delta_hr=round(median_d, 1),
        p90_delta_hr=int(round(p90_d)),
        mean_delta_hr_pct=round(mean_d_pct, 1),
        total_autonomic_burden=int(round(tab)),
        tab_normalized=int(round(tab_norm)),
        tab10=int(round(tab10)),
